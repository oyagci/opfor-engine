...
void Batch::AddMesh(std::vector<GLfloat> const &positions, std::vector<GLfloat> const &normals,
...
void Batch::Build()
{
    ...
    const size_t bufferSize =
        (_positions.size() + _normals.size() + _uvs.size() + _tangents.size() + _materials.size()) * sizeof(GLfloat);
    glGenBuffers(1, &_vbo);
