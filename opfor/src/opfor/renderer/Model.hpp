...
class Model
{
  private:
    ...
    std::optional<std::vector<unsigned int>> TinyProcessNode(tinygltf::Node const &node, tinygltf::Model const &model,
                                                             std::vector<std::string> const &materials)
    {
        ...
        if (node.mesh >= 0)
        {
            ...
            for (auto const &primitive : mesh.primitives)
            {
                ...
                auto getVertex =
                    [&](std::string const &attributeName) -> std::optional<std::pair<size_t, const float *>> {
                ...
                auto indices = reinterpret_cast<const GLushort *>(
                ...
                {
                    current.addTriangle({static_cast<GLuint>(indices[i * 3 + 0]),
    ...
    std::optional<std::vector<unsigned int>> TinyLoader(std::string const &path)
    {
        ...
        std::vector<std::string>
            pbrMaterials; // Save the materials' index to be able to set the material of the mesh later
