...
public:
	...
	static bool GetKeyUp(KeyCode keyCode) { return Get()->GetKeyUp_Impl(keyCode); }

	/// Returns the status of the key
	static KeyStatus GetKey(KeyCode keyCode) { return Get()->GetKey_Impl(keyCode); }

	/// Get the mouse position on the screen (x, y)
	static glm::vec2 GetMousePosition() { return Get()->GetMousePosition_Impl(); }

	static glm::vec2 GetMouseRelativePosition() { return Get()->GetMouseRelativePosition_Impl(); }

	/// Returns true if the mouse button has been pressed
	static bool GetMouseButtonDown(MouseButton btn) { return Get()->GetMouseButtonDown_Impl(btn); }

	/// Returns true if the mouse button has been released
	static bool GetMouseButtonUp(MouseButton btn) { return Get()->GetMouseButtonUp_Impl(btn); }

	/// Returns the status of the mouse button
	static KeyStatus GetMouseButton(MouseButton btn) { return Get()->GetMouseButton_Impl(btn); }

	/// Clear the input buffer for the next frame
	static void Flush() { return Get()->Flush_Impl(); }
};
