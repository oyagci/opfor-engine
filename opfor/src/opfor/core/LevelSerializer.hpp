	...
	static std::string Serialize()
	{
		...
		for (size_t n = 0; n < entities.size(); n++)
		{
			...
			if (ent->HasComponents<TransformComponent>())
			{
				...
				serialized[n]["transform"]["position"] = { transform.position.x, transform.position.y, transform.position.z };
				serialized[n]["transform"]["rotation"] = { transform.rotation.w, transform.rotation.x, transform.rotation.y, transform.rotation.z };
