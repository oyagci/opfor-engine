...
constexpr GLenum ToGlBufferBit(CopyTarget target)
{
    switch (target)
    {
        case CopyTarget::ColorBufferBit: return GL_COLOR_BUFFER_BIT;
        case CopyTarget::DepthBufferBit: return GL_DEPTH_BUFFER_BIT;
        case CopyTarget::StencilBufferBit: return GL_STENCIL_BUFFER_BIT;
        default: OP4_CORE_UNREACHABLE("Invalid CopyTarget");
    }
}
...
constexpr GLenum ToGlRendererCap(RendererCaps cap)
{
	switch (cap)
	{
	case RendererCaps::Blend: return GL_BLEND;
	case RendererCaps::CullFace: return GL_CULL_FACE;
	case RendererCaps::DepthTest: return GL_DEPTH_TEST;
	default:
...
constexpr GLint ToGlDataFormat(DataFormat dataFormat)
{
	...
	case DataFormat::RGB16F:  return GL_RGB16F;
	case DataFormat::Depth:   return GL_DEPTH_COMPONENT;
	default:
...
constexpr GLenum ToGlTextureParameterType(TextureParameterType textureType)
{
	switch (textureType)
	{
	case TextureParameterType::MignifyFilter: return GL_TEXTURE_MIN_FILTER;
	case TextureParameterType::MagnifyFilter: return GL_TEXTURE_MAG_FILTER;
	case TextureParameterType::WrapR:         return GL_TEXTURE_WRAP_R;
	case TextureParameterType::WrapS:         return GL_TEXTURE_WRAP_S;
	case TextureParameterType::WrapT:         return GL_TEXTURE_WRAP_T;
	default:
...
constexpr GLint ToGlTextureParameterValue(TextureParameterValue value)
{
	...
	case TextureParameterValue::NearestMipmapNearest: return GL_NEAREST_MIPMAP_NEAREST;
	case TextureParameterValue::NearestMipmapLinear:  return GL_NEAREST_MIPMAP_LINEAR;
	case TextureParameterValue::LinearMipmapNearest:  return GL_LINEAR_MIPMAP_NEAREST;
	case TextureParameterValue::LinearMipmapLinear:   return GL_LINEAR_MIPMAP_LINEAR;
	case TextureParameterValue::ClampToEdge:          return GL_CLAMP_TO_EDGE;
	case TextureParameterValue::ClampToBorder:        return GL_CLAMP_TO_BORDER;
	case TextureParameterValue::Repeat:               return GL_REPEAT;
	case TextureParameterValue::MirroredRepeat:       return GL_MIRRORED_REPEAT;
	default:
