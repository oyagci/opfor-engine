...
WindowsInput *WindowsInput::_UniqueInstance = nullptr;

const std::unordered_map<int, KeyCode> WindowsInput::_GlfwKeycodes = {
...
const std::unordered_map<int, MouseButton> WindowsInput::_GlfwMouseButtons = {
	{ GLFW_MOUSE_BUTTON_1, MouseButton::Button1 },
	{ GLFW_MOUSE_BUTTON_2, MouseButton::Button2 },
	{ GLFW_MOUSE_BUTTON_3, MouseButton::Button3 },
...
WindowsInput::WindowsInput()
{
	...
	glfwSetCursorPosCallback(_WindowHandle, &WindowsInput::MouseCallback);
}

void WindowsInput::RegisterKey(int action, int key, int mods)
{
...
void WindowsInput::RegisterKeyPress(KeyCode keyCode, int /* mods */)
{
	_PressedKeys.insert(keyCode);
}

void WindowsInput::RegisterKeyRelease(KeyCode keyCode, int /* mods */)
{
	_ReleasedKeys.insert(keyCode);
}

void WindowsInput::RegisterMousePos(float x, float y)
{
	_LastMousePosition = _MousePosition;
	_MousePosition = { x, y };
}

void WindowsInput::RegisterMouseButton(int glfwButton, int action, int mods)
{
...
void WindowsInput::RegisterMouseButtonPress(MouseButton button, int /* mods */)
{
	_PressedMouseButtons.insert(button);
}

void WindowsInput::RegisterMouseButtonRelease(MouseButton button, int /* mods */)
{
	_ReleasedMouseButtons.insert(button);
}

KeyCode WindowsInput::GetGLFWKeyCode(int key) const
{
...
MouseButton WindowsInput::GetGLFWMouseButton(int glfwButton) const
{
...
bool WindowsInput::GetKeyDown_Impl(KeyCode keyCode)
{
	return _PressedKeys.find(keyCode) != _PressedKeys.end();
}

bool WindowsInput::GetKeyUp_Impl(KeyCode keyCode)
{
	return _ReleasedKeys.find(keyCode) != _ReleasedKeys.end();
}

KeyStatus WindowsInput::GetKey_Impl(KeyCode keyCode)
{
...
bool WindowsInput::GetMouseButtonDown_Impl(MouseButton btn)
{
	return _PressedMouseButtons.find(btn) != _PressedMouseButtons.end();
}

bool WindowsInput::GetMouseButtonUp_Impl(MouseButton btn)
{
	return _ReleasedMouseButtons.find(btn) != _ReleasedMouseButtons.end();
}

KeyStatus WindowsInput::GetMouseButton_Impl(MouseButton btn)
{
...
void WindowsInput::KeyCallback(GLFWwindow *, int key, int /* scancode */, int action, int mods)
{
	_UniqueInstance->RegisterKey(action, key, mods);
}

void WindowsInput::MouseCallback(GLFWwindow *, double xpos, double ypos)
{
	_UniqueInstance->RegisterMousePos(static_cast<float>(xpos), static_cast<float>(ypos));
}

void WindowsInput::MouseButtonCallback(GLFWwindow *, int button, int action, int mods)
{
