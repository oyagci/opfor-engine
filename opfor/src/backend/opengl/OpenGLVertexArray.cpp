...
void OpenGLVertexArray::AddVertexBuffer(SharedPtr<VertexBuffer> vertexBuffer)
{
    ...
    for (uint32_t i = 0; i < static_cast<uint32_t>(elements.size()); i++)
    {
        ...
        glVertexAttribPointer(i, static_cast<GLsizei>(elem.GetComponentCount()), ShaderDataTypeToOpenGLBase(elem.Type),
                              elem.Normalized, layout.GetStride(), reinterpret_cast<void *>(offset));
    }
